...
if(isset($_POST['stuid'])==true && isset($_POST['password']))
{
$stuid=mysql_real_escape_string(strtoupper($_POST['stuid']));
$stupasswd=mysql_real_escape_string(md5($_POST['password']));
$sel_user="SELECT * FROM students WHERE stuid='$stuid' and encrypted='$stupasswd'";
...
$check_user=mysqli_num_rows($run_user);
	...
	else
	{
		...
		echo "<script>window.alert('Wrong UserID or Password');window.open('login.php','_self')</script>";
	}
}
else{
	echo "<h3 style='font-family:georgia;color:red;'>Wrong UserId or Password...<img src='images/loading.gif'></h3>";
	echo "<script>window.alert('Invalid Credentials');window.open('login.php','_self');</script>";
}
?>
